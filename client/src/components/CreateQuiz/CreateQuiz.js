...
export default class CreateQuiz extends React.Component {
   ...
   constructor(props) {
       super(props);
       this.state = {
           categories: ['Humour', 'Science', 'Technologies', 'Sports', 'Histoire', 'Musique'],
           categoryVal: 'Humour',
   ...
   render() {
       return (
           <div className="create-quiz-wrapper">
               ...
               <div className="main">
                   <div className="header">Créer un Quiz</div>
                   <div className="form card">
                       <input className="input" onChange={e => this.setState({name: e.target.value})} value={this.state.name} placeholder="Nom du Quiz *" />
                       <br></br>
                       <input className="input" onChange={e => this.setState({imgUrl: e.target.value})} value={this.state.imgUrl} placeholder="URL de votre image *" />
                       <br></br>
                       ...
                       <div className="checkbox">
                           <span>Doit être connecté pour répondre à ce quiz</span>
                       ...
                       {this.state.questions.map((ques, idx) => (
                           <div className="question" key={idx}>
                               ...
                               <div>Bonne réponse: {ques.correctAnswer}</div>
                               <div>Nombre de réponses: {ques.answers.length}</div>
                               <span className="btn delete" onClick={() => this.removeQuestion(ques)}>Supprimer</span>
                           </div>
                       ))}

                       <div className="questions">
                           <div className="add-question" onClick={() => this.setState({addQuestion: true})}>Ajouter des questions</div>
                       </div>

                       <span onClick={() => this.saveQuiz()} className="btn save-quiz">Sauvegarder</span>
